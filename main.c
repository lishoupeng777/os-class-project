...
int main() {
    ...
    printf("Do you want to reformat the filesystem? (1: Yes / 0: No): ");
    ...
    if (choice == 1) {
        ...
        if (load_from_disk()) {
            printf("Successfully restored system state from existing virtual disk.\n");
            init_structures();
            // 重新初始化用户（因为用户信息没有保存到虚拟磁盘）
            strcpy(users[0].u_name, "root");
            strcpy(users[0].u_passwd, "root");
            users[0].u_uid = 0;
            users[0].u_gid = 0;
            memset(users[0].u_ofile, -1, sizeof(users[0].u_ofile));
            users[0].u_cwd = iget(ROOT_INODE);
            user_count = 1;
        } else {
            printf("Virtual disk file not found, creating new system...\n");
            init_structures();
            format_disk();
        }
    }
